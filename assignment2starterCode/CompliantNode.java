...
public class CompliantNode implements Node {
    ...
    public void receiveFromFollowees(Set<Candidate> candidates) {
        ...
        for (int i = 0; i < followees.length; i++) {
            if (followees[i] && !senders.contains(i)) /* if there is no tx from a followee then we blacklist him*/
                blackListed[i] = true; /*this only handles the malicious nodes of 'dead type' */
        }
